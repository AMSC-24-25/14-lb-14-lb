...
__device__  long field0_index(unsigned long x, unsigned long y)
{
    return NX * y + x;
}

__device__  long scalar_index(unsigned long x, unsigned long y)
{
    return NX * y + x;
}

__device__  long fieldn_index(unsigned long x, unsigned long y, unsigned long d)
{
    return (ndir - 1) * (NX * y + x) + (d - 1);
}

__device__  void lid_driven_cavity(unsigned long x, unsigned long y, double &r, double &u, double &v)
{
...
__global__ void lid_driven_cavity(double *r, double *u, double *v)
{
    ...
    for (unsigned long y = index; y < NY; y+= stride)
    {
        for (unsigned long x = 0; x < NX; ++x)
        {
            long sidx = scalar_index(x, y);
            lid_driven_cavity(x, y, r[sidx], u[sidx], v[sidx]);
        }
    }
}

__global__ void init_equilibrium(double *f0, double *f1, double *r, double *u, double *v)
{
    long index = blockIdx.x * blockDim.x + threadIdx.x;
    long stride = blockDim.x * gridDim.x;
...
__global__ void stream_collide_save(double *f0, double *f1, double *f2, double *r, double *u, double *v, bool save)
{
    long index = blockIdx.x * blockDim.x + threadIdx.x;
    long stride = blockDim.x * gridDim.x;
    ...
    for (unsigned long y = index; y < NY; y+= stride)
    {
        for (unsigned long x = 0; x < NX; ++x)
        {
            unsigned long xp1 = (x + 1) % NX;
            unsigned long yp1 = (y + 1) % NY;
            unsigned long xm1 = (NX + x - 1) % NX;
            unsigned long ym1 = (NY + y - 1) % NY;
...
__global__ void compute_flow_properties(unsigned long t, double *r, double *u, double *v, double *prop)
{
...
void report_flow_properties(unsigned long t, double *rho, double *ux, double *uy)
{
    double prop[4];
    compute_flow_properties<<<3072, 64>>>(t, rho, ux, uy, prop);
...
void save_scalar(const char *name, double *scalar, unsigned long n)
{
    ...
    long ndigits = floor(log10((double)NSTEPS) + 1.0);
