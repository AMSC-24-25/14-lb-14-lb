...
LBM::LBM(LBM::VelocitySet::StandardSet vSet, LBM::partition_config par_conf, LBM::dimensions d, double nu) : N(d), nu(nu)
{
    ...
    double partition_x_size = double(N.x) / par_conf.world;
    x_loc = {std::lround(partition_x_size * par_conf.rank), std::lround(partition_x_size * (par_conf.rank + 1)), par_conf.rank != 0, par_conf.rank != par_conf.world - 1};
    x_len = x_loc.end - x_loc.start + x_loc.left_pad + x_loc.right_pad;
    x_len_no_pad = x_loc.end - x_loc.start;
    node_id = par_conf.rank;
...
{
    for (std::function<void(unsigned int, unsigned int, unsigned int, Eigen::VectorXd &, LBM &)> bc : BoundaryConditions)
        bc(x, y, z, f, *this);
}
